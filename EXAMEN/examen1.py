...
def analizar_codigo():
    ...
    for tok in tokens:
        ...
        result_tree.insert("", 'end', values=(tok.value, palabra_reservada, identificador, operador, numero, simbolo))

ventana = tk.Tk()
ventana.title("Analizador Léxico con Tkinter")

frame_entrada = ttk.Frame(ventana)
frame_entrada.pack(fill="both", expand=True)

entrada_codigo = ScrolledText(frame_entrada, width=40, height=10, wrap=tk.WORD)
entrada_codigo.pack(fill="both", expand=True)

boton_analizar = tk.Button(frame_entrada, text="Analizar Código", command=analizar_codigo)
boton_analizar.pack(pady=5)

frame_salida = ttk.Frame(ventana)
frame_salida.pack(fill="both", expand=True)

result_tree = ttk.Treeview(frame_salida, columns=("Token", "Palabra Reservada", "Identificador", "Operador", "Número", "Símbolo"), show="headings")
